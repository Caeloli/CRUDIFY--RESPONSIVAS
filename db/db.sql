...
CREATE TABLE authorization_request (
    ...
    affected_email VARCHAR(50),
    affected_type INT,
    chat_id VARCHAR(30)
);

CREATE TABLE authorization_allow (
    allow_id SERIAL PRIMARY KEY,
    request_id_fk INT REFERENCES authorization_request(request_id) ON DELETE CASCADE,
    user_id_fk INT REFERENCES users(user_id) ON DELETE CASCADE,
    is_allowed BOOLEAN NOT NULL
...
-- Insert default data
INSERT INTO user_type (user_type) VALUES ('mt'), ('ma');
INSERT INTO states (state_name) VALUES ('active'), ('notify'), ('expired');

-- Function to set responsive files state
CREATE OR REPLACE FUNCTION set_responsive_files_state()
RETURNS TRIGGER AS $$
BEGIN
    ...
    EXCEPTION
        WHEN OTHERS THEN
            RAISE EXCEPTION 'Error updating state for responsive file %: %', NEW.resp_id, SQLERRM;
    END;
END;
$$ LANGUAGE plpgsql;

-- Trigger to set responsive files state before insert
DROP TRIGGER IF EXISTS set_responsive_files_state_trigger ON responsive_files;
CREATE TRIGGER set_responsive_files_state_trigger
BEFORE INSERT ON responsive_files
FOR EACH ROW
EXECUTE FUNCTION set_responsive_files_state();

-- Function to insert authorization allow
CREATE OR REPLACE FUNCTION insert_authorization_allow()
RETURNS TRIGGER AS $$
BEGIN
    INSERT INTO authorization_allow (request_id_fk, user_id_fk, is_allowed)
...
-- Trigger to insert authorization allow after insert
