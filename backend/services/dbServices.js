...
async function postgreSQLInitValuesDB() {
  ...
  try {
    ...
    if (!adminUser) {
      ...
      console.log("Creted a new admin account ", result.email);
      globals.adminUser = result;
    }

    //Update status to admins that where before
        ...
        try {
          ...
          if (userInfo) {
            ...
            console.log(
              "User with email: ",
              email,
              "couldn't be found in the active directory"
            );
          }
        } catch (err) {
    ...
    if (!notifData) {
      ...
      console.log(
        "Eliminando todos los registros y creando registro nuevo ",
        result.notif_data_id,
        " con bot: ",
        result.bot_id
      );
    }
